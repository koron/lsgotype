...
func (w *walker) listPublic(path, name string, pkg *srcdom.Package) error {
	...
	for _, tname := range pkg.TypeNames() {
		typ, ok := pkg.Type(tname)
		if !ok || !typ.IsPublic() {
			continue
		}
		fmt.Printf("type: %s.%s\n", name, typ.Name)
	}
	for _, fname := range pkg.FuncNames() {
		fn, ok := pkg.Func(fname)
		if !ok || !fn.IsPublic() {
			continue
		}
		fmt.Printf("func: %s.%s\n", name, fn.Name)
	}
	for _, vname := range pkg.ValueNames() {
		v, ok := pkg.Value(vname)
		if !ok || !v.IsPublic() {
			continue
		}
...
func (w *walker) genVimSyntax(path, name string, pkg *srcdom.Package) error {
	n := len(pkg.Types)
	if n == 0 {
		fmt.Printf("\" skipped %s package: no types\n", name)
		return nil
	}
	if shouldSkip(name) {
		fmt.Printf("\" skipped %s package: marked as IGNORE\n", name)
		return nil
	}
...
func run(ctx context.Context) error {
	...
	switch mode {
	case "syntax":
		fmt.Printf("\" generated by github.com/koron/lsgotype with %s\n\n", root)
